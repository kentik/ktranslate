package rule

import (
	"fmt"
	"net"

	"github.com/kentik/patricia"
	tree "github.com/kentik/patricia/string_tree"
)

// compare two uint32s
func stringMatches(payload string, val string) bool {
	return payload == val
}

type StringAddressRule struct {
	treeV4 *tree.TreeV4
	treeV6 *tree.TreeV6
}

// NewIPAddressRules returns a new IPAddressRules
func NewStringAddressRule() *StringAddressRule {
	return &StringAddressRule{
		treeV4: tree.NewTreeV4(),
		treeV6: tree.NewTreeV6(),
	}
}

// AddIPAddress adds a rule match by IP Address
func (r *StringAddressRule) AddIPAddress(ipAddress string, val string) error {
	v4Addr, v6Addr, err := patricia.ParseIPFromString(ipAddress)
	if err != nil {
		return fmt.Errorf("Error parsing IP address: %s", err)
	}

	if v4Addr != nil {
		r.treeV4.Add(*v4Addr, val, stringMatches)
	} else if v6Addr != nil {
		r.treeV6.Add(*v6Addr, val, stringMatches)
	}

	return nil
}

// Check checks which Matches match the input IP
func (r *StringAddressRule) Check(ip net.IP) string {
	ret := ""
	var found bool

	if ipv4 := ip.To4(); ipv4 != nil {
		address := patricia.NewIPv4AddressFromBytes(ipv4, 32)
		found, ret = r.treeV4.FindDeepestTag(address)
	} else {
		address := patricia.NewIPv6Address(ip.To16(), 128)
		found, ret = r.treeV6.FindDeepestTag(address)
	}
	if found {
		return ret
	}

	return ""
}
