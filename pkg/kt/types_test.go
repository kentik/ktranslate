...
func TestUnifyFlatten(t *testing.T) {
	...
	InputTestingUnify[0].Pair = InputTestingUnify[1].Flatten()
	res := InputTestingUnify[0].Flatten()

	assert.Equal(InputTestingUnify[0].CustomStr["src_endpoint"], res["src_endpoint"])
	assert.Equal(InputTestingUnify[1].CustomStr["src_endpoint"], res["pair_src_endpoint"], "map: %v", res)
	assert.Equal(int64(InputTestingUnify[1].L4DstPort), res["pair_l4_dst_port"], "map: %v", res)
